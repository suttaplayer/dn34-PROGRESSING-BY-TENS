...
title Progression from Decline to Release

state "Ignorance" as IgnoranceState
state "Craving" as CravingState
state "Unskillful\nQualities" as UnskillfulQualities {
    state "Passion" as PassionState
    state "Aversion" as AversionState
    state "Delusion" as DelusionState
    state "Sloth & Drowsiness" as SlothState
    state "Restlessness & Anxiety" as RestlessnessState
    state "Uncertainty" as UncertaintyState
    state "Conceit" as ConceitState
    state "Stinginess" as StinginessState
}
state "Suffering & Stress" as SufferingState
state "Lower Realms\n(Rebirth)" as LowerRealms

state "Skilful\nQualities" as SkillfulQualities {
    state "Dispassion" as DispassionState
    state "Goodwill" as GoodwillState
    state "Discernment" as DiscernmentState
    state "Alertness" as AlertnessState
    state "Mindfulness" as MindfulnessState
    state "Persistence" as PersistenceState
    state "Confidence" as ConfidenceState
    state "Generosity" as GenerosityState
}
state "Right View" as RightView
state "Unbinding &\nRelease" as UnbindingState

[*] --> IgnoranceState : Not discerning Dhamma
IgnoranceState --> CravingState : Ignorance as requisite condition
CravingState --> UnskillfulQualities : Craving stitches to becoming

UnskillfulQualities --> SufferingState : Unskillful leads to harm & pain
SufferingState --> LowerRealms : Leads to bad destinations
LowerRealms --> IgnoranceState : Cycle of wandering-on

UnskillfulQualities --> SkillfulQualities : Abandonment & Development

SkillfulQualities --> RightView : Culmination of skillful qualities
RightView --> UnbindingState : Path to cessation of stress

IgnoranceState --> DiscernmentState : With right view, ignorance purges
CravingState --> DispassionState : Cessation of delight
PassionState --> DispassionState : Fading of passion
AversionState --> GoodwillState : Develop goodwill
DelusionState --> DiscernmentState : Fading of ignorance

SlothState --> AlertnessState : Apply effort
RestlessnessState --> Equanimity : Attending to equanimity
UncertaintyState --> ConfidenceState : Conviction helps overcome uncertainty
ConceitState --> DispassionState : Abandon conceit
StinginessState --> GenerosityState : Practice generosity

DispassionState --> UnbindingState : Leads to cessation
GoodwillState --> UnbindingState : Awareness-release
DiscernmentState --> UnbindingState : Leads to right ending of stress
AlertnessState --> SkillfulQualities
MindfulnessState --> SkillfulQualities
PersistenceState --> SkillfulQualities
ConfidenceState --> SkillfulQualities
GenerosityState --> SkillfulQualities

UnbindingState --> UnbindingState : No further becoming
