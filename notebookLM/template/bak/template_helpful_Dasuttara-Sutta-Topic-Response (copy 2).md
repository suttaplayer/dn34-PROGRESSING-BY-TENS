...
for (int i=0; i < <user-specified-progress-by-number>; i++) {        
            ...
            * Let the practitioner get a feel for how to relate to <user-specified-topic>[i]
}
...
2.  When generating the diagrams consider:
        * Cognitive Load: Too many elements overwhelm the human brain's ability to process information simultaneously (i.e., if more than  elements then create another diagram).
        * Visual Clutter: A large number of nodes makes the diagram visually cluttered and difficult to parse quickly (i.e., if more than 18 link elements then create another diagram).
...
3. If generating multiple diagrams, consider:
