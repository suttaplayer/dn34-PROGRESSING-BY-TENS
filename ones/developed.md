...
title Mindfulness Immersed in the Body: Progressive Cultivation
scale 1.0

state "Initial State" as Initial

state "Virtue_&_Straight_Views_Established" as Virtue
state "Heedful_&_Resolute" as Heedful

state "Mindfulness_Immersed_in_Body_Practice" as MIB_Practice {
  state "Mindful_In-Out_Breathing" as MIB_Breathing
  state "Abandoning_Household_Resolves" as MIB_Resolves
  state "Mind_Gathers_&_Settles" as MIB_Gathers
}

state "Jhāna_Attainment" as Jhana_Stages {
  state "Joy_Arises" as Joy
  state "Rapture_Arises" as Rapture
  state "Body_Calm" as BodyCalm
  state "Pleasure_Arises" as Pleasure
  state "Mind_Concentrated" as Concentration

  state "First_Jhāna" as J1
  state "Second_Jhāna" as J2
  state "Third_Jhāna" as J3
  state "Fourth_Jhāna" as J4
}

state "Effluent-Free_Release" as Release <<end>>

[*] --> Initial

Initial --> Virtue : Cultivate virtue &\nstraight views
Virtue --> Heedful : Practice consistently
Heedful --> MIB_Practice : Go to seclusion,\nset body erect, establish mindfulness

MIB_Practice --> Jhana_Stages : Mind gathers,\njoy, rapture, calm, pleasure, concentration

Jhana_Stages --> Release : Cessation of delight,\nclinging, becoming

J1 --> J2 : Stilling directed thoughts\n& evaluations
J2 --> J3 : Fading of rapture
J3 --> J4 : Abandoning pleasure & pain

Jhana_Stages -down-> Jhana_Stages : [Relishing/Clinging to feelings]
Jhana_Stages -down-> MIB_Practice : [Hindrances/Unskillful qualities]
MIB_Practice -down-> MIB_Practice : [Sluggishness/Scattered mind]
...
@startuml
hide empty description
title Components of Developed Mindfulness
...
class Mind {
  ...
  + bright : Boolean
  + unblemished : Boolean
  + free_from_defects : Boolean
  + steady : Boolean
  + imperturbable : Boolean
}

class Virtue {
  + well_purified : Boolean
  + untorn : Boolean
  + unbroken : Boolean
  + unspotted : Boolean
  + unsplattered : Boolean
  + liberating : Boolean
  + praised_by_observant : Boolean
  + ungrasped_at : Boolean
  + conducive_to_concentration : Boolean
}
...
class Mindfulness_Immersed_in_Body {
  + focus_on_body : Boolean
  + mindful_breathing : Boolean
  + sensitive_to_entire_body : Boolean
  + calming_bodily_fabrications : Boolean
}

class Jhanas {
  + first_jhana : Boolean
  + second_jhana : Boolean
  + third_jhana : Boolean
  + fourth_jhana : Boolean
}
...
class Effluents {
  + defile_mind : Boolean
  + lead_to_renewed_becoming : Boolean
  + give_trouble : Boolean
  + ripen_in_stress : Boolean
  + lead_to_future_birth_aging_death : Boolean
}

class Hindrances {
  + sensual_desire : Boolean
  + ill_will : Boolean
  + sloth_&_drowsiness : Boolean
  + restlessness_&_anxiety : Boolean
  + uncertainty : Boolean
}

class Unbinding_Release {
  + deathless : Boolean
  + effluent_free : Boolean
  + no_I_making : Boolean
  + no_mine_making : Boolean
  + no_further_becoming : Boolean
}

Practitioner --|> Mind : possesses
Practitioner --|> Virtue : cultivates
Practitioner --|> Views : holds

Mindfulness_Immersed_in_Body --|> Mind : refines
Mind --|> Joy : leads_to
Mind --|> Rapture : leads_to
Mind --|> Calm : leads_to
Mind --|> Pleasure : leads_to
Mind --|> Concentration : leads_to

Jhanas --|> Mind : represents_heightened_states_of
Concentration --|> Jhanas : enables

Mind <.. Effluents : defiled_by
Mind <.. Hindrances : impeded_by
Effluents <.. Hindrances : contribute_to

Unbinding_Release <.. Mind : achieved_by_purified
Unbinding_Release <.. Effluents : absence_of
